...
function create_noise() {
    micromamba run -n SE3nv RFdiffusion/scripts/run_inference.py inference.output_prefix=${OUTPUT_PATH} inference.input_pdb=${INPUT_PDB} ${CONTIG_MAP} inference.num_designs=1 diffuser.partial_T=${PARTIAL_T}
    pdb_tofasta ${INPUT_PDB} >rfdiffusion.fasta
    FASPR/FASPR -i ${OUTPUT_PATH}_0.pdb -o ${FASPR_OUTPUT_PATH} -s rfdiffusion.fasta
    micromamba run -n attnpacker python attnpack.py --inputpdb ${FASPR_OUTPUT_PATH} --outputpath ${ATTN_OUTPUT_PATH} --numcores 8
    podman run --rm -v $(pwd):/home openstructure:latest compare-structures --model ${FASPR_OUTPUT_PATH} --reference $2 --output $3/ost_scores.json --lddt --local-lddt --tm-score --rigid-scores
    podman run --rm -v $(pwd):/home openstructure:latest compare-structures --model ${ATTN_OUTPUT_PATH} --reference $2 --output $3/ost_scores.json --lddt --local-lddt --tm-score --rigid-scores
}

# fold with full full side-chain, fold only backbone

# CASP14/ or CASP13/
for FILE in CASP13/*.fasta; do

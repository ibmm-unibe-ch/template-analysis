...
function clean_pdb() {
    pdb_sort $1 | pdb_tidy | pdb_chain -A >temp_packing_${IDENTIFIER}
    cp temp_packing_${IDENTIFIER} $1
    pdbfixer temp_packing_${IDENTIFIER} --output=temp_packing_${IDENTIFIER} --replace-nonstandard --add-residues
    cat temp_packing_${IDENTIFIER} |grep ^ATOM > $1
...
vmd -dispdev text -e generate_topology.tcl ${DATASET}
for FILE in ${DATASET}/*.pdb; do
    FILENAME=$(basename $FILE .pdb)
    PARENT_DIR="$(dirname "$FILE")"
    echo $FILENAME

...
def read_scores(scores_path:Path) -> dict:
    with open(scores_path,"r") as file:
        data = json.load(file)
    output = {"rmsd":data.get("rmsd", None), "tm_score":data.get("tm_score", None),"lddt":data.get("lddt", None), }
    return output
...
def read_colabfold_scores(colabfold_path:Path) -> dict:
    with open(colabfold_path,"r") as file:
        data = json.load(file)
    output = {"plddt": mean(data["plddt"]), "max_pae": data["max_pae"], "ptm":data["ptm"]}
    return output

def get_colabfold_result(parent_path:Path, identifier:str, msa_mode:str, template_id:str) -> dict:
    folder_path = parent_path/f"{identifier}_{msa_mode}_{template_id}"
    file_names = [filename for filename in os.listdir(folder_path) if filename.startswith(f"{identifier}_scores_rank_001_")]
    if len(file_names)==0:
        return None
    file_name = file_names[0]
    colabfold_results = read_colabfold_scores(folder_path/file_name)
    basic_info = {"identifier": identifier, "msa_mode":msa_mode, "template_id":template_id, "tool": "ColabFold"}
...
def get_results_identifier(parent_path:Path, identifier:str) -> dict:
    ...
    for (msa_mode, template_id) in [(msa_mode, template_id) for msa_mode in ["single", "full"] for template_id in ["none", "omega", "truth", "comb", "esm"]]:
...
def get_results_folder(folder_path:Path):
    ...
    for file in os.listdir(folder_path):
        if not str(file).endswith(".fasta"):
            continue
        identifer = str(file)[:4]
        output =output+get_results_identifier(folder_path, identifer)

...
def read_ost_scores(path:Path) -> dict:
    ...
    if read_json["status"]=="FAILURE":
        return None
    output = {"lddt": read_json.get("lddt", None), "ost_rmsd": read_json.get("rmsd", None), "tm_score":read_json.get("tm_score", None)}
...
def get_refinement_results_folder_monomer(folder_path: Path, save_path: Optional[Path]=None):
    output = []
    for file in os.listdir(folder_path):
        if not str(file).endswith(".fasta") or str(file).endswith("_single.fasta"):
            continue
